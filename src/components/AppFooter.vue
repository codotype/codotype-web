<template>
  <footer class="footer bg-dark text-light">
    <div class="container-fluid text-right">

      <!-- Place this tag where you want the button to render. -->
      <a class="github-button" href="https://github.com/aeksco" data-size="large" data-show-count="true" aria-label="Follow @aeksco on GitHub">Follow @aeksco</a>

      <a class="github-button" href="https://github.com/codotype/codotype-web" data-size="large" data-show-count="true" aria-label="Star codotype/codotype-web on GitHub">Star</a>

      <!-- <a target="_blank" href="https://github.com/aeksco"> -->
        <!-- <i class="fa fa-fw fa-github mr-2"></i> -->
        <!-- aeksco -->
      <!-- </a> -->
    </div>
  </footer>
</template>

<script>
export default {}
</script>

<style lang="sass">

  footer.footer
    position: absolute
    bottom: 0
    width: 100%
    line-height: 60px

</style>
